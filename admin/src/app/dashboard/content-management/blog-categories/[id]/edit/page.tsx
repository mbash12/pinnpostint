...
export default function EditBlogCategoryPage({ params }: EditBlogCategoryPageProps) {
    ...
    const { data, isLoading } = useBlogCategory(id);

    if (isLoading) return <LoadingState message="Loading category..." />;
    if (!data) return <ErrorState message="Category not found" />;

    return <EditBlogCategoryForm data={data} />;
}

function EditBlogCategoryForm({ data }: { data: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result?.success) {
                showAlert("Category Updated", "Blog category has been successfully updated.", "success");
                setTimeout(() => router.push("/dashboard/content-management/blog-categories"), 1500);
            } else {
    ...
    return (
        <FormLayout breadcrumb="Content management" title="Edit Blog Category">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <FormActions
                    cancelHref="/dashboard/content-management/blog-categories"
