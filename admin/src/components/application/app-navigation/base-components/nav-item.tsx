...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    ...
    const labelElement = (
        <span
            className={cx(
                "text-left text-md font-semibold text-secondary transition-inherit-all group-hover:text-secondary_hover",
                truncate && "truncate",
    ...
    if (type === "collapsible") {
        return (
            ...
            >
                ...
                <ChevronDown aria-hidden="true" className="ml-2 size-4 shrink-0 stroke-[2.5px] text-fg-quaternary in-open:-scale-y-100" />
            </summary>
        );
    }
